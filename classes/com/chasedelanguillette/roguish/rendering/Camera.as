...
package com.chasedelanguillette.roguish.rendering
{
	...
	*/
	public class Camera
	{
		...
		public var bUpdated:Boolean = false;
		...
		*/
		...
		/**
		*	Tells whether a particular point is on camera.
		*/
		public function onCamera(x:int, y:int):Boolean
		{
			if (x >= _nViewOffsetX && x <= _nViewOffsetX + _nViewWidth && y >= _nViewOffsetY && y <= _nViewOffsetY + _nViewHeight) return true;
		...
		*/
		...
		{
			var nx:int = x - _nHalfViewWidth;
			var ox:int = _nViewOffsetX;
			
			_nViewOffsetX = nx;
			validateOffsetX();
			
			if ( nx != ox ) bUpdated = true;
		};
		...
		*/
		...
		{
			var ny:int = y - _nHalfViewHeight;
			var oy:int = _nViewOffsetY;
			
			_nViewOffsetY = y - _nHalfViewHeight;
			validateOffsetY();
			
			if ( ny != oy ) bUpdated = true;
		};
		...
		public function panx(xoffset:int):void
		{
			_nViewOffsetX += xoffset;
			validateOffsetX();
		};
		
		public function pany(yoffset:int):void
		{
			_nViewOffsetY += yoffset;
			validateOffsetY();
		};
		
		private function validateOffsetX():void
		{
			if ( _nViewOffsetX < 0 ) _nViewOffsetX = 0;
			if ( _nViewOffsetX > _nWorldBoundaryRight ) _nViewOffsetX = _nWorldBoundaryRight;
		};
		
		private function validateOffsetY():void
		{
